...
public class RankingBasedExtensionReasonerWeightedAttacksCat extends AbstractExtensionReasoner {
    ...
    public boolean getAttacks(DungTheory bbase, Collection<Extension<DungTheory>> extensions) {
        var attacks = extensions.stream()
                .map(ext -> bbase.getAttackers(ext))
                .flatMap(list -> list.stream()).collect(Collectors.toList());
        System.out.println("Max" + attacks.stream().mapToDouble(att ->
    ...
    private static boolean isAdmissible(DungTheory bbase, Map<Argument, Double> ranking, Extension<DungTheory> e) {
        ...
        //Admissible Dung: Zahl der Angriffe auf Argument muss kleiner sein als Gewicht der Angriffe der Extension auf Angreifer
